...
uint32_t AdapterWrapper::authKeyReply(const uint8_t keyType, const uint8_t *key)
{
    ...
    if (err_code != NRF_SUCCESS)
    {
        get_logger()->debug("{} Error calling sd_ble_gap_auth_key_reply: {}", role(),
                            testutil::errorToString(err_code));
    }
                                             ...
                                             const uint8_t minKeySize, const uint8_t maxKeySize)
{
    ...
    if (err_code != NRF_SUCCESS)
    {
        get_logger()->debug("{} sd_ble_gap_sec_params_reply, {}", role(),
                            testutil::errorToString(err_code));
    }
...
uint32_t AdapterWrapper::securityParamsReply(const ble_gap_sec_keyset_t &keyset)
{
    ...
    if (err_code != NRF_SUCCESS)
    {
        get_logger()->debug("{} sd_ble_gap_sec_params_reply failed, {}", role(),
                            testutil::errorToString(err_code));
    }
...
uint32_t AdapterWrapper::startCharacteristicDiscovery()
{
    ...
    if (err_code != NRF_SUCCESS)
    {
        get_logger()->debug("{} sd_ble_gattc_characteristics_discover, {}", role(),
                            testutil::errorToString(err_code));
    }
...
uint32_t AdapterWrapper::startDescriptorDiscovery()
{
    ...
    {
        get_logger()->debug("{} No characteristic handle specified.", role());
        return NRF_ERROR_INVALID_STATE;
    }
    ...
    if (err_code != NRF_SUCCESS)
    {
        get_logger()->debug("{} sd_ble_gattc_descriptors_discover failed, err_code: {}", role(),
                            testutil::errorToString(err_code));
    }
...
uint32_t AdapterWrapper::writeCCCDValue(const uint16_t cccdHandle, const uint8_t value)
{
    ...
    if (err_code != NRF_SUCCESS)
    {
        get_logger()->debug("{} sd_ble_gattc_write, {}", role(), testutil::errorToString(err_code));
    }
...
uint32_t AdapterWrapper::setBLEOptions()
{
    ...
    if (err_code != NRF_SUCCESS)
    {
        get_logger()->debug("{} sd_ble_opt_set, {}", role(), testutil::errorToString(err_code));
    }
                                   ...
                                   const char *message)
{
    ...
    {
        get_logger()->debug("std::system_error in statusHandler: {}", e.what());
    }
}
...
void AdapterWrapper::eventHandler(adapter_t *adapter, ble_evt_t *p_ble_evt)
{
    ...
    {
        get_logger()->debug(
            "In eventHandler, not able to find adapter to invoke event handler on.");
    }
    catch (std::system_error &e)
    {
        get_logger()->debug("std::system_error in eventHandler: {}", e.what());
    }
}
                                ...
                                const char *log_message)
{
    ...
    {
        get_logger()->debug("In logHandler, not able to find adapter to invoke log handler on.");
    }
    catch (std::system_error &e)
    {
        get_logger()->debug("std::system_error in logHandler: {}", e.what());
    }
}
