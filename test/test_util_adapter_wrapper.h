...
{
  public:
    AdapterWrapper(const Role& role, const std::string& port, const uint32_t baudRate, const uint16_t mtu = 0)
      : m_role(role)
      , m_port(port)
    {
    ...
    {
        const auto err_code =
          sd_ble_gap_connect(m_adapter, address, &(scratchpad.scan_param), &(scratchpad.connection_param)
#if NRF_SD_BLE_API >= 5
    ...
    bool error() { return m_async_error; }
    ...
    uint32_t setAdvertisingData(const std::vector<uint8_t>& advertisingData)
    {
        ...
        auto err_code = sd_ble_gap_adv_data_set(m_adapter, advertisingData.data(),
                                                static_cast<uint8_t>(advertisingData.size()), sr_data, sr_data_length);
#endif
        ...
        std::copy(advertisingData.begin(), advertisingData.end(), scratchpad.adv_report_adv_data_buffer);
        ...
        auto err_code = sd_ble_gap_adv_set_configure(m_adapter, &(scratchpad.adv_handle), &(scratchpad.adv_report_data),
                                                     &(scratchpad.adv_params));
#endif
    ...
    uint32_t startServiceDiscovery(const uint8_t type, const uint16_t uuid)
    {
        ...
        const auto err_code =
          sd_ble_gattc_primary_services_discover(m_adapter, scratchpad.connection_handle, start_handle, &srvc_uuid);
    ...
    uint32_t writeCharacteristicValue(const uint16_t characteristicHandle, const std::vector<uint8_t>& data)
    {
        ...
        NRF_LOG(role() << " Writing to connection_handle: " << testutil::asText(scratchpad.connection_handle)
    ...
    adapter_t* unwrap() { return m_adapter; }
    ...
    void processEvent(const ble_evt_t* p_ble_evt)
    {
        ...
        if (eventId >= BLE_GAP_EVT_BASE && eventId <= BLE_GAP_EVT_LAST) {
            const auto logUnprocessed = [this, &eventId]() {
                NRF_LOG(role() << " Unprocessed GAP event: 0x" << std::setfill('0') << std::setw(2) << std::hex
                               << (uint32_t)eventId);
            };
            ...
            const auto logUnprocessed = [this, &eventId]() {
                NRF_LOG(role() << " Unprocessed GATTC event: 0x" << std::setfill('0') << std::setw(2) << std::hex
                               << (uint32_t)eventId);
            };
            ...
            const auto logUnprocessed = [this, &eventId]() {
                NRF_LOG(role() << " Unprocessed GATTS event: 0x" << std::setfill('0') << std::setw(2) << std::hex
                               << (uint32_t)eventId);
            };
    ...
    void setStatusCallback(const StatusCallback& statusCallback) { m_statusCallback = statusCallback; }

    void setLogCallback(const LogCallback& logCallback) { m_logCallback = logCallback; }

    void setEventCallback(const EventCallback& eventCallback) { m_eventCallback = eventCallback; }

    void setGattcEventCallback(const GattcEventCallback& callback) { m_gattcEventCallback = callback; }

    void setGattsEventCallback(const GattsEventCallback& callback) { m_gattsEventCallback = callback; }

    void setGapEventCallback(const GapEventCallback& callback) { m_gapEventCallback = callback; }
